  ...
  return (
    <AppShell title="ホーム">
      <section className="space-y-7 py-4">
            ...
            <div className="wa-panel space-y-3 p-4 text-sm text-nezumi">
              ...
              <div className="rounded-lg border border-[var(--border)] bg-kinari/40 p-3 text-xs leading-relaxed text-nezumi">
                ...
                <ul className="mt-1 list-disc space-y-1 pl-5">
                  ...
                  <li>提出前に必ずご自身で事実確認・修正を行ってください。</li>
                  <li>同意いただけない場合は作成を開始できません。</li>
                </ul>
              </div>

              <button
                type="button"
                className="inline-flex items-center gap-2 text-sm font-semibold text-ai underline-offset-2 hover:underline focus-visible:wa-focus"
                onClick={() => setIsTermsExpanded((prev) => !prev)}
                aria-expanded={isTermsExpanded}
                aria-controls="full-terms"
              >
                {isTermsExpanded ? '詳細を閉じる' : '詳細を見る'}
              </button>

              {isTermsExpanded && (
                <div id="full-terms" className="max-h-72 space-y-2 overflow-y-auto pr-2 leading-relaxed">
                  <p>
        ...
        <p className="text-center text-xs text-nezumi/70">© Acoru合同会社</p>
      </section>
    </AppShell>
  );
}
